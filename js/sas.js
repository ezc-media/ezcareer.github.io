$(document).ready(function () {
  ...
  const course_name_des =
    "全称STATISTICAL ANALYSIS SYSTEM，是一门主要运用的领域就在于统计 以及分析的计算机语言，在全球100多个国家地区普及程度极高，在行 政、科研、商业等领域也起着至关重要的作用。";
  const price_origin = "899";
  const price_discount = "799";
  const course_open_date = "7月";
  const course_link =
    "https://ezcareer.shop/collections/2020/products/2020-4-sas";
  const course_points = [
    {
      title: "基础串讲",
    ...
    },
    {
      title: "千题冲刺",
      des:
        "将历年考试核心重点梳理，整理总结SAS考试核心知识点，极大缩短了学生备考时间， 提升考试通过率",
  ...
  const target_group = [
    ...
    },
    {
      title: "金融从业人员",
      des:
        "目前已经入职银行等相关行业，担任入门级工作，但是希望继续晋升和发展的在职员工",
    },
    {
      title: "新移民",
      des:
        "刚刚来到加拿大，希望学习新技能，增加就业机会的、提高发展前景、提升自身实力的新移民",
    },
    {
      title: "其他行业",
      des:
        "目前已经就职在其他行业，但是依然想进入投行金融、咨询行业的其他行业人员",
    },
  ];
  ...
  const about_course =
    "Easy Career的SAS课程可以帮助学生在学习或者第一次零基础接触这门软件时避免走弯路，把有限的精力放在必考的考点上，而不是盲目的复习。通过线上线下同步学习，将每一个data coding，题目及重大考点逐个掌握。";
...
`,
  ...
  const video_info = {
    src: "https://www.youtube.com/watch?v=S526dYyOFAg",
    cover: assets_route + "images/courses/SAS/cover.PNG",
  };

  const teacher_name = ["Leo老师"];
  ...
  $(`#courseTable`).html(
    createCourseTable(course_list_tab, course_list, course_link)
  );
